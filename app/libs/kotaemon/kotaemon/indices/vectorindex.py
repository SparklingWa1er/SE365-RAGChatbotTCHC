...
class VectorRetrieval(BaseRetrieval):
    ...
    def run(
        ...
        # Normalize empty list to None: [] means "no files selected" in UI,
        # treat same as None (no filter) so queries run across all documents.
        scope = kwargs.pop("scope", None) or None
        # Normalize empty filters: UI sends filters={} when no filter is set;
        # passing {} to llama_index VectorStoreQuery raises AttributeError
        # (expects MetadataFilters object). Remove when empty.
