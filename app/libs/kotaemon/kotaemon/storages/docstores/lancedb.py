...
class LanceDBDocumentStore(BaseDocumentStore):
    ...
    # Khi scope lớn hơn ngưỡng này thì BỎ prefilter "id in (...)". LanceDB FTS
    # (Rust) dựng chuỗi filter với hàng chục nghìn id -> panic "memory allocation
    # failed" trên Windows. Khi search "all", scope = toàn bộ chunk nên filter vô
    # nghĩa -> bỏ qua, FTS chạy trên cả bảng. Chỉ giữ filter khi user chọn ÍT file.
    FTS_PREFILTER_LIMIT = 1000

    def query(
        self, query: str, top_k: int = 10, doc_ids: Optional[list] = None
    ) -> List[Document]:
        if doc_ids and len(doc_ids) <= self.FTS_PREFILTER_LIMIT:
