...
# ============================================================
#  OVERRIDE — chatbot Thủ tục hành chính công (tiếng Việt)
#  Embedding mặc định: bge-m3 (local, free) | LLM: Azure gpt-4o
#  Đặt ở cuối file để chắc chắn ghi đè các default phía trên.
# ============================================================

# 1) Embedding multilingual-e5-large chạy local qua fastembed (onnxruntime, không cần torch)
#    Giữ làm fallback khi .env KHÔNG cấu hình Azure embedding. (1024 chiều)
KH_EMBEDDINGS["e5-multilingual"] = {
    "spec": {
        "__type__": "kotaemon.embeddings.FastEmbedEmbeddings",
...
# 2) Embedding mặc định: ưu tiên Azure text-embedding-3-large (3072 chiều) nếu .env có
#    cấu hình; nếu không thì fallback về e5-multilingual local.
_emb_default = "azure" if "azure" in KH_EMBEDDINGS else "e5-multilingual"
for _name, _cfg in KH_EMBEDDINGS.items():
    _cfg["default"] = _name == _emb_default

# 3) Azure là LLM mặc định (nếu .env đã cấu hình Azure)
if "azure" in KH_LLMS:
    for _name, _cfg in KH_LLMS.items():
        _cfg["default"] = _name == "azure"

# 4) Ngôn ngữ trả lời mặc định: tiếng Việt (nếu được hỗ trợ)
if "vi" in SUPPORTED_LANGUAGE_MAP:
    SETTINGS_REASONING["lang"]["value"] = "vi"
